...
- **Revoking access is `users.disabled`** (migration `0017`, `npm run revoke -- <email>`, `--undo` to restore) — never a `DELETE` of the `users` row, which is destructive and blocked by five FK children. The flag is checked in *four* places because a session lasts 90 days and the `.ics` feed is unauthenticated: `findUserIdByEmail`, `getSessionUserId` (it JOINs `users` for exactly this reason — don't "simplify" that join away), `serveCalendarFeed`, and the push/email cron queries. A new path that reads user data needs the check too.
