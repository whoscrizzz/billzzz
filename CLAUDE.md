...
Forward-only numbered SQL files (`0001`–`0012`), applied with the `db:migrate:*` scripts. Tables: `users`, `sessions`, `magic_links`, `subscriptions`, `payment_records`, `push_subscriptions`, `notification_log`, `notification_attempts`, `passkey_credentials`, `webauthn_challenges`, `auth_rate_limits`. Schema changes are additive `ALTER TABLE … ADD COLUMN` with NULL defaults so existing rows keep working; never rewrite an applied migration.
