...
- **`GET /bills-api/auth/verify` must stay 405.** Verification is POST-only so email scanners prefetching links can't burn magic-link tokens. If a diff makes this route accept GET, that's a regression, not a feature.
